# IMG-043 — GNSS 변위 계측 개념도 (v6 ai-reviewed)

> **판정:** **v6 PASS (ai-reviewed)** — book/GNSS.pdf · ZIP-AUD-07 · hero 품질 개선
...
## 최종 생성 프롬프트 (v6 — ai-reviewed · book/GNSS.pdf · LOGO-01)

> **기록:** [docs/216](../../docs/216-IMG-043-v6-재생성-기록.md)

```text
[docs/36 §1.0] 계측 목적: GNSS 기준국·이동국 3D 변위 모니터링 시스템 구성.

좌 65%: 사면 단면(또는 댐 크레스트) — 기준국(안정 지반·영향권 밖) GNSS 안테나,
이동국 #1 #2 #3 각 GNSS 안테나, ΔX·ΔY·ΔZ(또는 동·북·수직) 변위 벡터.
위성 GPS/GLONASS/BeiDou 간단 도식(과도한 사실 렌더 금지).
우 35%: 블록도 — GNSS 안테나 → 수신기 → 무선통신(LTE) → 중앙 서버.
RTK/차분 보정정보 화살표(기준국→이동국, 광파 시준 점선 금지).
선택: 태양광·배터리(원격 무인).
한글 라벨: 기준국, 이동국, GNSS 안테나, 중앙 서버, 3D 변위, 무선 통신.
금지: 광파기·프리즘·제조사 로고·단일 안테나·앱 UI·사람.
