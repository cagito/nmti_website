/**
 * Mark Phase 2 P2 heroes as pending-external migration (doc 31 Phase 2).
 * Preserves migrationStatus=completed after register-external-figure.
 *
 * Usage: node scripts/seed-phase2-migration.mjs
 */
import { readFileSync } from 'fs';
import { join, dirname } from 'path';
import { fileURLToPath } from 'url';
import { atomicWriteUtf8, readJsonSafe } from './lib/atomic-write.mjs';

const ROOT = join(dirname(fileURLToPath(import.meta.url)), '..');
const phase2Path = join(ROOT, 'scripts', 'phase2-p2-heroes.json');
const registryPath = join(ROOT, 'scripts', 'image-review-registry.json');

const phase2 = JSON.parse(readFileSync(phase2Path, 'utf8'));
const registry = readJsonSafe(registryPath, {});

let marked = 0;
let skipped = 0;

for (const hero of phase2.heroes) {
  const id = hero.id;
  const reg = registry[id];
  if (!reg) {
    console.warn(`skip ${id}: not in registry`);
    continue;
  }
  if (reg.migrationStatus === 'completed' && (reg.productionMethod === 'ai-reviewed' || reg.productionMethod === 'cad')) {
    skipped++;
    continue;
  }
  if (reg.productionMethod === 'ai-reviewed' || reg.productionMethod === 'cad') {
    reg.migrationStatus = 'completed';
    reg.migrationPhase = 'P2';
    skipped++;
    continue;
  }
  reg.migrationStatus = 'pending-external';
  reg.migrationPhase = 'P2';
  marked++;
}

atomicWriteUtf8(registryPath, `${JSON.stringify(registry, null, 2)}\n`);
console.log(`seed-phase2-migration: ${marked} pending-external, ${skipped} skipped (completed/already external)`);
