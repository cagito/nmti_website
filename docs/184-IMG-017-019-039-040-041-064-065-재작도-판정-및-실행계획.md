...
**판정:** **조건부 보류** — v5 픽셀·문구는 INTERP-01 충족으로 보임. **redline Q1~Q8 육안 서명** 후 PASS 유지.
...
| 항목 | 내용 |
|------|------|
| **노드** | `fields/soft-ground` · hero는 **IMG-020** 등 분야별 전용 |
| **파일** | `IMG-019_연약지반-계측-전체-개념도_성토침하간극수압측방유동.webp` |
| **정본** | [redline v3](../ImageWorks/NMTI_Engineering_Image_Prompt_Package_v1/redlines/IMG-019_redline_v3_외부PNG.md) · SOFT-LAYOUT-01 · [27 연약지반](./image-knowledge/27-연약지반·압밀-계측.md) |
| **현행** | v3 · `render-img019-soft-ground-layout.py` · 2026-06-29 PASS |
| **핵심 리스크** | ① **육안 redline 미서명** ② 파일명「전체 개념도」↔ redline「설치 배치 단면도」 불일치 ③ image-knowledge「019=callout 개요만」 vs 다목적 나열 위험 ④ `docs/109` 정본 **미존재**(redline 참조 깨짐) |

**판정:** **재작도 권장 (P1)** — 7종 중 **유일하게 최근 ai-reviewed v5 미적용** · SOFT-LAYOUT-01 전수 미검증.
...
**선행 작업:** `docs/109-IMG-019-…-표현-표준.md` **복구 또는 redline 정본을 27+redline v3로 통합**
...
**판정:** **조건부 보류** — redline FAIL 시 v6.

**v6 보완 포인트:** 지중 매설 = **짧은 매설 well·지표 마커** (IPI 장관 아님) · 발파 영향권은 IMG-097 참조만 유지
...
| **현행 (v6)** | 육측\|케이슨\|해측 · 조위·G.W.L · ai-reviewed 2026-06-30 |
| **잔여 리스크** | ① **육안 redline 미서명** ② 육측 **데이터로거** 소형 박스 — P0-3 함체 규격 미달 가능 ③ 계기 **과밀 callout** ④ AGENTS「064 REGENERATE 권장」이력 ⑤ 구조 변위 상세는 **084**와 중복 점검 필요 |

**판정:** **재작도 검토 (P1)** — hero·고노출 · v6 직후에도 **출판·과밀·로거** 리스크 잔존.
...
| 019 | 재작도 후 `renderScript` 제거 또는 legacy 표기 · caption/파일명 정합 |
...
| 질문 | 답 |
|------|-----|
| **7종 전부 다시 그려야 하나?** | **아니오.** 즉시 재작도 **권장은 019(필수급)·064(검토급)** 2종. **040은 유지.** 나머지 4종은 **육안 redline 후 결정**. |
| **전체 112 WebP 중 최우선?** | **1~2:** [185 S0](./185-IMG-024-011-최우선-재작도-실행계획.md) ✅ · **3~4:** [186 S1](./186-IMG-016-004-3-4순위-재작도-실행계획.md) ⏳ |
| **가장 먼저 할 일?** | **S0:** IMG-024 v5 → IMG-011 v5 · 이후 Phase 0 redline (7종) |
| **문서 선행?** | **IMG-019** — `docs/109` 복구 또는 redline v3를 단일 정본으로 승격 |
