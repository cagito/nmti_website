...
### 0.0f 전수감사 출판·대외·PASS 게이트 (2026-06-29)

> [130-수정계획](./130-기술자료-이미지-전수감사-디자인원칙-수정계획.md) · [180-zip재검수](./180-technology-이미지-전수-재검수-수정계획.md) · [51 §0.6](./51-계측-도면-검수-공통-원칙.md) · [131 §8](./131-P0-이미지-재작성-공통-프롬프트-정본.md)

| 코드 | 규칙 |
|------|------|
| **LOGO-01** | AI/CAD **생성 원본** 로고·워터마크 **금지** → `watermark-figures.bat` · `npm run watermark:figures` 후처리만 — **[183 정본](./183-이미지-생성-워터마크-금지-정본.md)** |
| **LEGEND-01** | 범례·캡션 = 본도 실재 항목만 |
| **EXT-01** | PASS·REGENERATE·P0·BRI-01·DAM-01·EXC-01·「검수용」**대외 금지** |
| **FOOT-01** | 하단 장문 설명·조립표 **금지** (1~2줄 참고만) |
| **PASS-GATE** | **5게이트** — 1개라도 틀리면 PASS 불가 ([182](./182-분야별-계측이미지-PASS-기준.md)) |

**P0 8종 큐 (zip 재검수):** [180 §3](./180-technology-이미지-전수-재검수-수정계획.md) · WebP 파일명 **풀네임** 유지.

### 0.0g 계측 질문·PASS 5게이트 (2026-06-29 · zip 112 WebP)

> **정본:** [180 §0~§1](./180-technology-이미지-전수-재검수-수정계획.md) · [181](./181-이미지별-계측오류-금지조건-정본.md)
...
| # | PASS 게이트 |
|---|-------------|
| 1 | 계측기가 **실제 설치 가능한 위치**에 있는가? |
| 2 | 계측기가 측정하는 **물리량**이 맞는가? |
| 3 | **힘·변위·수두·침하·장력·압축력 방향**이 맞는가? |
| 4 | **기준점·대상점·측정축**이 해석상 성립하는가? |
| 5 | 도면 문구가 **확정/추정/예시**를 구분하는가? |

**하나라도 NO → `PASS` 금지** (`REGENERATE` 또는 `MAJOR_FIX`).
