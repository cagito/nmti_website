...
- **IMG-004** = 어스앵커 하중계 설치 개념도 (`retaining-excavation/anchor`). **S1 4순위** — v7 PASS · `requiresReaudit` · 목표 **v8 REGENERATE** — [186](./docs/186-IMG-016-004-3-4순위-재작도-실행계획.md) · [54](./docs/54-IMG-004-어스앵커-하중계-설치-표현-표준.md) · [83 ANC-CLOCK](./docs/83-어스앵커-하중계-ANC-CLOCK-정본.md) · LC=굴착측 두부·반력판–헤드 사이·**사선 동축(1~7시, NOT 수평)** · T/P 분리 · 버팀보 LC 혼동 금지
...
- **IMG-016** = 원호활동면 계측 해석도 (`fields/slope/slip-surface`). **S1 3순위** — v5 PASS · `requiresReaudit` · 목표 **v6** — [186](./docs/186-IMG-016-004-3-4순위-재작도-실행계획.md) · INTERP-01 · **추정 원호활동면**만 · 최대변위≠활동면 · IPI 단독 확정 금지
